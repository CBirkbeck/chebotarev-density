...
theorem count_primes_above_with_frobenius_eq_sigma
    ...
    Nat.card {𝔓 : Ideal (𝓞 L) // ∃ (hp : 𝔓.IsPrime) (hP : 𝔓.LiesOver 𝔭)
      ...
      * orderOf σ * Nat.card C.carrier
      = Nat.card Gal(L/K) := by
  rw [mul_right_comm,
    count_frobenius_eq_sigma_mul_card_carrier K L σ C _hσ 𝔭 _hpr _hnz hunr _hCfrob]
  exact card_primesAbove_mul_orderOf_eq K L σ C _hσ 𝔭 _hpr _hnz hunr _hCfrob
