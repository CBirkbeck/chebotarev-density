...
\begin{proof}
  ...
  By the higher-power tail bound \cref{lem:prime-zeta-higher-tail-bounded}
  and the analytic-at-$1$ identity
  $\log\zeta_K(s) = \sum_\fp N\fp^{-s} + O(1)$, the partial-sum
  $\sum_\fp N\fp^{-s}$ differs from $\log\zeta_K(s)$ by a bounded
  function. Since $\zeta_K$ has a simple pole at $s=1$,
  $\log\zeta_K(s) \le \log\bigl(1/(s-1)\bigr) + O(1)$.
\end{proof}
