...
theorem cyclotomic_frobenius_acts_as_norm_power
    ...
    [𝔭.IsPrime] [NeZero 𝔭] (hunr : UnramifiedIn K L 𝔭) (hcop : (Ideal.absNorm 𝔭).Coprime m)
...
theorem autToPow_frobeniusClass_out
    ...
    {ζ : L} (hζ : IsPrimitiveRoot ζ m) (𝔭 : Ideal (𝓞 K)) [𝔭.IsPrime] [NeZero 𝔭]
...
private theorem unramifiedIn_intermediateField
    (K L : Type*) [Field K] [NumberField K] [Field L] [NumberField L] [Algebra K L] [IsGalois K L]
    (F : IntermediateField K L) [IsGalois K F]
    (𝔭 : Ideal (𝓞 K)) [NeZero 𝔭] (hunr : UnramifiedIn K L 𝔭) :
...
private theorem frobeniusClass_fixedField_eq_one
  ...
  rw [frobeniusClass_eq_mk_of_isArithFrobAt K (↥F) 𝔭
    (unramifiedIn_intermediateField K L F 𝔭 hunr)
...
private theorem finrank_mul_unramified_coprime_le_univ
  ...
  have hsplit : ∀ 𝔭 : Ideal (𝓞 K), 𝔭.IsPrime → 𝔭 ≠ ⊥ → UnramifiedIn K L 𝔭 →
    ...
    fun 𝔭 h𝔭p h𝔭ne h𝔭unr h𝔭cop ↦ by
      haveI := h𝔭p
      haveI : NeZero 𝔭 := ⟨h𝔭ne⟩
      exact ⟨card_primesOver_fixedField_eq_finrank K L H 𝔭 h𝔭unr
