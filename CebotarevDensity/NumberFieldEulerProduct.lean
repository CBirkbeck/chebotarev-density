...
/-- Re-indexing of exponent vectors when a new index `a ∉ s` is inserted:
`(↥(insert a s) → ℕ) ≃ ℕ × (↥s → ℕ)`, recording the value at `a` and the restriction to `s`. -/
...
/-- The product `∏_{i ∈ insert a s} g i ^ e i` splits off the new factor at `a`, with the
remaining product re-indexed through `insertPiEquiv`. -/
...
/-- The combinatorial heart of the finite Euler-factor identity, over an abstract finite index.
For `g : ι → ℂ` with every `‖g i‖ < 1`, the finite product of geometric series
`∏ i ∈ s, (1 - g i)^{-1}` equals the `tsum` over exponent vectors `e : ↥s → ℕ` of
`∏ i, g i ^ e i`, and the family is norm-summable. Proved by induction on `s`, splitting off the
new factor with `HasSum.mul`. -/
private lemma finsetGeometricProd_summable_and_hasSum {ι : Type*} (g : ι → ℂ)
  ...
  | empty =>
    ...
    · have h1 : (fun e : {i // i ∈ (∅ : Finset ι)} → ℕ => ‖∏ i ∈ Finset.attach ∅, g i.1 ^ e i‖)
          = fun _ => (1 : ℝ) := by funext e; simp [Finset.attach_empty]
      rw [h1]
      exact (hasSum_unique (fun _ : {i // i ∈ (∅ : Finset ι)} → ℕ => (1 : ℝ))).summable
    · have h1 : (fun e : {i // i ∈ (∅ : Finset ι)} → ℕ => ∏ i ∈ Finset.attach ∅, g i.1 ^ e i)
          = fun _ => (1 : ℂ) := by funext e; simp [Finset.attach_empty]
      rw [h1]
  ...
  | insert a s ha ih =>
    ...
    have hgeosum : Summable (fun n : ℕ => ‖g a ^ n‖) := by
      simp_rw [norm_pow]; exact summable_geometric_of_lt_one (norm_nonneg _) (hg a)
    ...
    · have heq : (fun e : {i // i ∈ insert a s} → ℕ => ‖∏ i ∈ (insert a s).attach, g i.1 ^ e i‖)
        ...
        funext e; rw [Function.comp_apply, prodInsertAttach g a s ha e]
      rw [heq]; exact (insertPiEquiv a s ha).summable_iff.mpr hHsum
    · have heq : (fun e : {i // i ∈ insert a s} → ℕ => ∏ i ∈ (insert a s).attach, g i.1 ^ e i)
          = (fun x : ℕ × ({i // i ∈ s} → ℕ) =>
              g a ^ x.1 * ∏ i ∈ s.attach, g i.1 ^ x.2 i) ∘ insertPiEquiv a s ha := by
        funext e; rw [Function.comp_apply, prodInsertAttach g a s ha e]
      rw [heq]; exact (insertPiEquiv a s ha).hasSum_iff.mpr hHhas

/-- A finite product of `natCast`-powers collapses to a single `natCast`-power of the product:
...
/-- For `1 < Re s`, the geometric ratio `N𝔭^{-s}` of a nonzero prime ideal has norm `< 1`
(since `N𝔭 ≥ 2`). This is the hypothesis feeding the geometric series in each Euler factor. -/
lemma norm_absNorm_cpow_neg_lt_one {s : ℂ} (hs : 1 < s.re)
  ...
  have hpos : 0 < Ideal.absNorm 𝔭.1 := by omega
...
`∏_{𝔭 ∈ S} (1 - N𝔭^{-s})^{-1} = Σ_{e : S →₀ ℕ} ∏_𝔭 N𝔭^{-(e 𝔭) · s}`, the sum running over
exponent vectors. Combined with `absNorm_prod_pow_of_primeIdeal`, the right-hand side is the
partial Dirichlet sum `Σ N𝔞^{-s}` over the ideals `𝔞 = ∏_𝔭 𝔭^{e 𝔭}` generated by `S`. -/
...
/-- **Ideal-norm bridge** for the finite Euler-factor identity: the norm of the ideal
`∏_𝔭 𝔭^{e 𝔭}` generated from `S` factors as `∏_𝔭 (N𝔭)^{e 𝔭}` (multiplicativity of `Ideal.absNorm`
plus `map_pow`). Hence `∏_𝔭 N𝔭^{-(e 𝔭)·s} = (N(∏_𝔭 𝔭^{e 𝔭}))^{-s}`, turning the right-hand side
of `prod_eulerFactor_eq_tsum_exponentVector` into the ideal Dirichlet partial sum. -/
...
/-- The summand of `prod_eulerFactor_eq_tsum_exponentVector` is the ideal-norm Dirichlet term:
`∏_𝔭 N𝔭^{-(e 𝔭)·s} = (N(∏_𝔭 𝔭^{e 𝔭}))^{-s}`. Bundles `prod_natCast_cpow` with the
`absNorm` bridge, so the partial Euler product is literally `Σ_e N(∏_𝔭 𝔭^{e 𝔭})^{-s}`. -/
...
/-- The ideal Dirichlet series `∑_𝔞 N𝔞^{-s}` over the nonzero ideals of `𝓞 L` has sum
`ζ_K(s)` for `1 < Re s`. Norm-grouped form of `dedekindZeta_eq_tsum_idealNormMultiplicity`
via `Equiv.sigmaFiberEquiv (absNorm)`, packaged as a `HasSum` (the `atTop` limit of partial
sums) so it can be compared with the finite Euler products. -/
private theorem hasSum_nonzeroIdeal_absNorm_cpow {s : ℂ} (hs : 1 < s.re) :
  ...
  set e := Equiv.sigmaFiberEquiv (fun I : NonzeroIdeal L => Ideal.absNorm I.1) with he
...
/-- The exponent of a single prime ideal `𝔮` in the ideal factorization of the prime power
`𝔭 ^ n` is `n` if `𝔮 = 𝔭` and `0` otherwise (distinct nonzero prime ideals are coprime). -/
...
open UniqueFactorizationMonoid in
/-- The exponent of a prime ideal `𝔮` in the ideal factorization of the product
`∏_{𝔭 ∈ S} 𝔭 ^ e 𝔭` distributes over the finset: it is `∑_{𝔭 ∈ S} factorization (𝔭^{e 𝔭}) 𝔮`
...
private theorem factorization_prod_primePow_apply
  ...
  | insert a s ha ih =>
    ...
    · rw [Finset.mem_image]; rintro ⟨x, -, hx⟩
      exact ha ((Subtype.mk.inj hx) ▸ x.2)
    · rw [Finset.mem_image]; rintro ⟨x, -, hx⟩
      exact ha ((Subtype.mk.inj hx) ▸ x.2)

open UniqueFactorizationMonoid in
/-- **Key factorization identity.** For an exponent vector `e` supported on the finset `S` of
nonzero prime ideals, the exponent of `𝔮` in the factorization of `∏_{𝔭 ∈ S} 𝔭 ^ e 𝔭` is `e 𝔮`
if `𝔮 ∈ S`, and `0` otherwise. This recovers the exponent vector from the product ideal, hence
both the injectivity of `e ↦ ∏_𝔭 𝔭 ^ e 𝔭` and the characterization of its range. -/
...
open UniqueFactorizationMonoid in
/-- The (normalized) prime factors of a nonzero ideal `𝔞`, packaged as a `Finset` of nonzero
prime ideals. Used to build the threshold finite set `S₀` in the `S ↑ ⊤` limit: once `S` contains
`primeFactorsOf 𝔞` for every `𝔞` in a finite mass-carrying set, those ideals lie in the range of
the exponent map. -/
...
private theorem mem_primeFactorsOf (𝔞 : NonzeroIdeal L) (p : Ideal (𝓞 L))
  ...
  exact ⟨⟨p, by rw [Multiset.mem_toFinset]; exact hp⟩, Finset.mem_attach _ _, rfl⟩
...
theorem dedekindZeta_eq_tprod_primeIdeal {s : ℂ} (hs : 1 < s.re) :
  ...
  classical
  set P := {𝔭 : Ideal (𝓞 L) // 𝔭.IsPrime ∧ 𝔭 ≠ ⊥} with hP
  set D : NonzeroIdeal L → ℂ := fun 𝔞 => (Ideal.absNorm 𝔞.1 : ℂ) ^ (-s) with hD
  -- `idealOfExp S e` is the ideal `∏_{𝔭 ∈ S} 𝔭 ^ e 𝔭`, a nonzero ideal.
  ...
  -- the ideal Dirichlet series sums to `ζ_K(s)`, and its norm-series is summable
  ...
  -- **injectivity** of `e ↦ ∏_𝔭 𝔭 ^ e 𝔭`: the factorization recovers the exponent vector
  have hinj : ∀ S : Finset P, Function.Injective (idealOfExp S) := by
    ...
    rw [← key e, ← key e', h]
  -- **range membership**: an ideal whose prime factors all lie in `S` is in the range
  have hmem : ∀ (S : Finset P) (𝔞 : NonzeroIdeal L),
    ...
    -- the product over `S` reconstructs `𝔞` (its prime factors all lie in `S`)
    ...
    exact hreconstruct
  -- **partial sum**: the finite Euler product is the Dirichlet sum over the range
  ...
  -- `‖∑_{range} D − ∑_{all} D‖ = ‖∑_{rangeᶜ} D‖ ≤ ∑_{rangeᶜ} ‖D‖ ≤ ∑_{Fᶜ} ‖D‖ < ε`
...
/-- For real `s > 1`, `ζ_K(s) = Σ_𝔞 N𝔞⁻ˢ` is a positive real (Sharifi Def 7.1.11, p.140).
Used for the `Real.log (ζ_K(s)).re` bookkeeping in the Euler-product log identity. -/
theorem dedekindZeta_re_pos_of_one_lt (s : ℝ) (hs : 1 < s) :
  ...
  -- each complex summand is the real cast of `g n`
  have key : ∀ n : ℕ,
  ...
  -- complex summability of the summand, transported to the real cast
  have hsumC : Summable
  ...
  -- compute the real part of the Dedekind zeta value
  have hre : (NumberField.dedekindZeta L (s : ℂ)).re = ∑' n : ℕ, g n := by
    ...
    simp
  rw [hre]
  -- positivity via `tsum_pos`: nonneg termwise, strictly positive at `n = 1`
